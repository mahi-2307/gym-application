...
@Service
@Slf4j
public class CoachWorkoutScheduleServiceImpl implements CoachWorkoutScheduleService {
    ...
    @Override
    public List<CoachBookDto> getCoachWorkoutSchedule(String authorizationHeader) throws UserNotFoundException {
        ...
        eav.put(":coachEmail", new com.amazonaws.services.dynamodbv2.model.AttributeValue().withS(email));
        ...
        List<CoachBookDto> coachWorkouts = coachBookEntities.stream()
                .filter(coachBookEntity -> coachBookEntity.getCoachEmail().equals(email))
                .map(coachBookEntity -> {
